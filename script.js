...
function handleInput() {
    ...
    gamesData.forEach(game => {
        ...
        const lowercaseReleaseTitle = game["Rus Title"]?.toLowerCase();

        if (lowercaseTitle.includes(userInput)) {
            suggestionsMap.set(game["Title"], game);
        }

        if (lowercaseReleaseTitle && lowercaseReleaseTitle.includes(userInput)) {
            suggestionsMap.set(game["Rus Title"], game);
        }
    });
...
function displayResult(game) {
    ...
    const matchingGames = gamesData.filter(dataGame => dataGame["Title"] === game["Title"]);
    ...
    matchingGames.forEach(matchingGame => {
        ...
        for (const key in matchingGame) {
            if (key !== "Rating" && key !== "Image" && key !== "Title" && key !== "Age Rating" && key !== "Play time" && key !== "Novel Description") {
                ...
                if (key === "Novel URL" || key === "Release URL" || key === "Release website") {
